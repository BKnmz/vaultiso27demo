...
def export_clause_to_word(cid, content, title_override=None,
    ...
    if tpl and tpl.exists():
        # Open template, clear body content — inherit styles, header, footer from template
...
@st.cache_data(show_spinner=False)
def export_clause_to_excel(cid, content, org_name=""):
    """Export a clause's tables as a styled .xlsx workbook.
    Used for tabular-primary clauses: 6.1.2, 6.1.3, 9.1, 9.2."""
    import openpyxl
    ...
    if not tables:
        ...
        ws.cell(row=1, column=1,
                value=f"{cid}: {CLAUSE_NAMES.get(cid, cid)}").font = Font(bold=True, size=13)
        for li, line in enumerate(content.splitlines(), 3):
        ...
        for tbl_data in tables:
            ...
            if org_name:
                ...
                banner = ws.cell(row=1, column=1,
                                 value=f"{org_name}  ·  {cid}: {CLAUSE_NAMES.get(cid, cid)}  ·  {tbl_data['title']}")
            ...
            for ri, row_vals in enumerate(tbl_data["rows"], header_row + 1):
...
ORG_JSON_SCHEMA = {
    "name": "", "industry": "", "size": "", "scope": "",
    "primary_processes": [], "locations": [], "departments": [],
    "regulatory_drivers": [], "legal_basis": [],
