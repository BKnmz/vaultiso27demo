...
def render() -> None:
    ...
    # Guard: org profile must have a name before generation is allowed (Bug I4)
    from core import load_org as _load_org
    _org_check = _load_org()
    if not _org_check.get("name", "").strip():
        st.warning(
            "Organization profile is not set up yet. "
            "Please fill in your company name and profile in **Settings → Organization Profile** before generating documents."
        )
        st.stop()
